...
def test_minimal_protobuf():
    ...
    try:
        # Try to import minimal test proto
        try:
            import minimal_pb2
            print_result(True, "Imported minimal_pb2")
        except ImportError:
            # If minimal.proto exists but minimal_pb2.py doesn't, try to compile it
            if os.path.exists("minimal.proto"):
                print("minimal.proto exists but minimal_pb2.py not found, compiling...")
                subprocess.run(["protoc", "minimal.proto", "--python_out=./"], check=True)
                import minimal_pb2
                print_result(True, "Compiled and imported minimal_pb2")
            else:
                # Create a minimal proto file and compile it
                print("Creating minimal test proto file...")
                with open("minimal_test.proto", "w") as f:
                    f.write("""
                    syntax = "proto2";
                ...
                subprocess.run(["protoc", "minimal_test.proto", "--python_out=./"], check=True)
                import minimal_test_pb2
                module = minimal_test_pb2
                print_result(True, "Created, compiled and imported minimal_test_pb2")
        
        # Create a test message
        message = minimal_pb2.MinimalMessage()
        message.text = "Test message for protobuf validation"
        message.number = 42
        
        # Serialize and deserialize
        serialized = message.SerializeToString()
        deserialized = minimal_pb2.MinimalMessage()
        deserialized.ParseFromString(serialized)
        
        # Check if the round trip worked
        round_trip_ok = (deserialized.text == message.text and 
                        deserialized.number == message.number)
...
def test_project_specific_proto():
    ...
    try:
        # Import the agents module which should be used in the project
        import conf.agents_pb2 as agents_pb2
        
        # Try to create an agent message
        agent = agents_pb2.Agent()
        agent.agent_type = "SEARCH_BOT"
        
        print_result(True, f"Successfully created agent message with type '{agent.agent_type}'")
        ...
        round_trip_ok = deserialized.agent_type == agent.agent_type
